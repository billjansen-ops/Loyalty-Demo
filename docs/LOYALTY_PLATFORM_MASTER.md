...
Molecules are the platform's core abstraction: they attach tenant-specific, typed values to
activities and members **without adding columns or code per tenant**. Delta's carrier/origin,
Insight's LICENSING_BOARD/REFERRAL_SOURCE, a hotel's room-type — all one system. The value is
stored right-sized and encoded in shared `{link_bytes}_data_{storage_size}` tables and decoded
back on read; nothing is duplicated as text. This is what makes the platform a reusable,
industry-agnostic engine rather than custom code per client. Business rules (bonuses, promotions,
signals, scoring) evaluate against molecule values, so behavior is data-driven, not hardcoded.

**The full mechanism — storage/encoding, per-type recipes, the silent-failure invariants, verified
exemplars, the helper functions, and the mandatory verification step — lives in one place:
[docs/MOLECULES.md](MOLECULES.md), the molecule authority and single source of truth.** Read it
before creating, editing, or reasoning about any molecule. This document intentionally does not
restate the mechanism: two copies drift, and the truth ends up in neither (which is exactly what
bit us before MOLECULES.md existed).
