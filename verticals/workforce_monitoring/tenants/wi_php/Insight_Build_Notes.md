...
*The matrix (Rev 1.1 §4):* Case Manager +Supersede at Tier 2 (D-11 — a corrected clinical document links as a version, never an unversioned duplicate; proven with a version-2 replace through the CM's own login); Program Administrator +Upload and +Legal-hold at Tier 2 (the role owns ingestion and holds — still no download, view-to-classify stands); Medical Director +Supersede at Tier 3 (D-10 — before this NOBODY could supersede a restricted document); Medical Director +Hold at org-level (a hold on an executed agreement no longer depends on an operations role).

*Unclassified is strictly Tier 2 (AC-11, "no tier-less state"):* stored at 2 from upload (v156 backfilled the old rows), and reachable ONLY through Tier 2 permissions — the S165 any-classifying-role upload rule is dead; PA's new U at Tier 2 is what makes fax ingestion work through the matrix itself. Registrant documents carry the same treatment (§6.2): a floor at Tier 2 even when classified to a Tier 1 type — no looser access model for intake material; they are also excluded from bulk export outright.

*Superseded is one intersection rule (D-13; AC-10 now Blocking; AC-12 new):* visible to the MD, the CM, and any role holding S at the document's tier — each only where that role holds V there. Resolves to T1 all three staff roles, T2 MD+CM, T3 MD alone, org-level MD+PA. Both S165 wrinkle implementations retired; the org case falls out of the rule with no special case. Enforced identically in the single-document resolver and the finder's SQL (one rule, two dialects, cross-checked in the test).

*Part 2 goes honest (her words: "intended behavior, not a defect"):* until the consent architecture exists no consent artifact can exist, so a flagged document is not downloadable by ANY role — the S165 interim Filed-consent-document unlock is retired, proven by linking a consent document and being refused anyway. Columns stay as plumbing (D-14).

*Break-glass splits (D-12):* the Medical Director ALONE records a grant — no deputy, no out-of-band path, an unavailable MD means no grant (accepted consequence, stated on the screen). MD or PA revokes, immediately, and the revocation writes its own audit event. The grant-list door answers can_record/can_revoke so the Emergency Access screen is role-aware. The grant notification is COUNTS-ONLY — no document titles, no reason text (her notification content rule: nothing clinical in any notification text, staff included).

*The audit log is a protected surface (§7.3) — and the leak check found real leaks:* /v1/audit/:table/:key was readable by ANY logged-in user (and, separately, had been THROWING since birth — it selected a column no audit table has ever had; fixed, field diffs now come from audit_change); /v1/audit/user-report had no role gate; /v1/audit/test could fabricate document events. Now: document-layer audit rows are MD+PA-only under rules (tenant admin under open), superusers have NO read path (break-glass covers the log), reading the log writes its own 'T' event — review of the trail is itself on the trail — and the dev door can never write protected-table events. NEW: GET /v1/audit/document-log + the Program Settings → Document Audit Log screen (admin_audit_review.html), walked live.

*Three per-program flags, present and OFF (v156, all three workforce tenants):* caseload_only (D-2), immediate_release (D-3 — wired into the filing door and proven both ways: ON auto-releases a filed lab with the same 'R' event, OFF restores the logged release action as the only path; read LIVE from the DB, never the boot cache), prescriber_portal (D-9, portal-phase). Erica's Q-1..Q-8 confirmations are now literally value changes.

*Proof:* test_document_access 182 → **200 asserts** green; test_access_rules_acceptance 148 → **207 asserts** green — AC-8 rewalked to Rev 1.1 (MD-only grants, counts-only notification, revocation event, no IHS path to the log) and AC-10/11/12 join the Blocking gate. Lint 0. SERVER_VERSION 2026.08.04.1046, DB v156. Not in scope, recorded: the Tier 3 individual CM grant mechanism (spec §3) still doesn't exist — CM at Tier 3 is simply denied; when it builds, the MD records it (Rev 1.1 change 13).

**Session 166 part 5 (2026-08-03) — MONITORING CORE STORY 3a: EXCUSED ABSENCES (v152), the channel-independent slice of story 3.**

*The shape:* an excused absence is a MARK on the selection row (who, why, when — Bill-epoch datetime), never a deletion; the selection stands in the record. An excused selection stops counting toward the paradigm quota, so the engine naturally re-rolls the test later in the period — the design default; Erica's pending re-roll-vs-drop answer is a one-line flip, recorded in the design doc. Reason required; one-way; approval belongs to the Medical Director and Case Manager (Bill's ruling), enforced through the program's role map under rules mode exactly like the release door — under open mode any staff login records it, since roles don't resolve until the program flips. Excusing TODAY's selection also clears the member_compliance missed-sweep pointer, so 5 PM never files a MISSED for a test the program excused. Chart-timeline surfacing through the member edit audit (the paradigm-assignment precedent). The clinic Testing tab's day detail gained the Excuse button and the EXCUSED display; excused rows lose the button.

*Also:* the v149 migration description's stale "3-byte link" prose corrected to the 4-byte integer reality (cosmetic — the code and console line were always right).

*Proof:* test_selection_engine grew 32 → **41 asserts**, green: refusals (no reason, double excuse), the mark's contents, the quota re-roll proven by the engine's own quotaMet counter dropping when a quota-filling selection is excused, and the missed-sweep pointer clear proven on Wisconsin's real compliance rows. Lint 0. Walked live on the sandbox: Marcus Webb's for-cause selection excused through the calendar's button, reason prompt and all. SERVER_VERSION 2026.08.03.1322, DB v152. Still waiting on Erica for the check-in channel (story 3's other half) and the positive-result review model (story 4).

**Session 166 part 3 (2026-08-03) — MONITORING CORE STORY 2: THE RANDOM SELECTION ENGINE (v151), AND THE DISCOVERY THAT SHAPED IT.**

*The discovery (recorded in the design doc §2b for Bill's read):* the platform already ran a selection loop — compliance's RANDOM_DRUG_TEST daily job (hardcoded 1-in-7 chance, minimum 2-day spacing, forced selection at 10 days) stamping member_compliance pointers, with DRUG_TEST_MISSED filing MISSED results and notifications at the 5 PM sweep. A second engine beside it would be exactly the two-variants drift BEFORE_YOU_WRITE forbids. **Resolution: the paradigm engine is the new BRAIN inside the existing BODY.** The RANDOM_DRUG_TEST job now runs the paradigm engine first — members with an active paradigm get their paradigm's math, logged to the new test_selection table AND stamped onto the same member_compliance pointers, so the existing missed-sweep and notification work for paradigm selections unchanged. The legacy 1-in-7 covers ONLY members with no paradigm (a NOT EXISTS in its query). One brain per member, never two; nothing changes anywhere until a paradigm is assigned; Wisconsin untouched until Erica configures paradigms.

*The engine:* per member per day, probability = tests remaining in the calendar period ÷ eligible days remaining (weekday rules + minimum gap respected; quota met = quiet; uniform spread, no pattern — the point of random testing). Clinicians excluded through the same custauth filter the legacy loop uses. v151's test_selection is the LOG the calendar reads (one row per member per day, source random/for-cause — member_compliance only ever held the last-selection pointer; a calendar needs history).

*The doors and the rule they enforce:* the selections list REFUSES future dates in plain English ("future test days are never named") — Bill's nobody-sees-the-future ruling lives server-side, not in screen politeness. The calendar returns actual counts through today and EXPECTED VOLUME ONLY beyond (remaining ÷ eligible days, spread over each assignment's eligible future days). Manual doors: the admin Run button presses the same engine function as the daily job; a for-cause selection requires member + reason, stamps the same pointers, and refuses a duplicate day.

*Screens, walked live on the sandbox:* Program Settings → Monitoring & Testing gained "Run selection now" (reports checked/selected/at-quota in plain English); clinic gained the **Testing tab** — a month calendar where past days are clickable (named list with source, reason, site, paradigm), today is outlined with its count, and future days show ~volume only. Walked end to end: engine run (1 analyzed, dice honestly said no at ~10% daily odds), for-cause selection on Marcus Webb, duplicate 409, future-date refusal, calendar with today's count and future ~0.1s on weekdays only.

*Proof:* tests/insight/test_selection_engine.cjs (102nd test, 32 asserts, green first run — every assert deterministic, no dice): certain-selection paradigm (quota ≥ days left → probability 1), gap block via a planted yesterday-selection, quota quiet, for-cause round trip, future-date refusal, calendar shape both sides of today, and the ONE-BRAIN proof on Wisconsin through the real job door — legacy 10-day force-select fires for an uncovered member, and the SAME member under a (quota-met) paradigm is untouched by the legacy rules entirely. test_monitoring_core re-run green; lint 0. SERVER_VERSION 2026.08.03.0313, DB v151. Stories 3-4 wait on Erica's channel/review answers; story 5 is the sandbox acceptance walk.

**Session 166 part 2 (2026-08-03) — MONITORING CORE STORY 1: THE CONFIGURATION SPINE (v150). Erica's WPHP rank 1 opens construction the same day her three design questions went out.**

*The design pass first:* docs/MONITORING_CORE_DESIGN.md drafted from her materials (her ranked build order's "build as one block", the organized wish list's monitoring cluster, our S152 gap analysis), discussed with Bill from the AMS airport, and settled as the contract. Bill's rulings recorded there: nobody sees the future (participant never sees a future test day; staff see today's selections; the calendar shows future volume only), excused absences staff-recorded and MD/CM-approved, collection sites a simple program-owned list, config in tables + events on the activity timeline, the selection engine as a daily scheduled job sharing one function with a manual Run (the MED_SCAN pattern), the watching layer on the existing signal machinery. THREE QUESTIONS TO ERICA (sent, filed in project_status/Monitoring_Core_Questions_2026-08.md): the pre-consent check-in channel (shapes story 3), excused-absence re-roll vs drop (default: re-roll), and the positive-result review workflow (shapes story 4's result record — the earliest-needed answer). None block stories 1-2. Explicitly OUT: lab-vendor integration (kickoff scoping — the manual entry door means records never wait), automated rescheduling (Washington clarification pending), external portals, billing.

*What shipped (v150):* collection_site + test_paradigm (tests_per_period × month/quarter/year, min_gap_days, weekdays_only — flexible DATA, kickoff enters Washington's real values) + member_paradigm — the assignment is TEMPORAL like member_tier: Bill-epoch start/end dates, ending stamps end_date, a new assignment is a new row, history never rewritten, exactly one open row per person. New vertical module monitoring.js: sites + paradigms CRUD (validations in plain English; duplicate codes 409; DELETES REFUSE while any assignment references the row — the S159 reward-object lesson — with tenant ownership checked FIRST so the refusal can never be a cross-tenant oracle, a real bug the new test caught on first run; retire via is_active; a retired paradigm cannot be newly assigned), GET/PUT /v1/members/:id/paradigm (site optional; {paradigm_code:null} ends; changes write the member 'E' audit so they surface on the chart timeline — the licensing-board precedent). Copier: test_paradigm copies as program policy config (the compliance_item precedent) + manifest part 'Testing paradigms'; collection_site + member_paradigm recorded not-copied (state content / operational history — the manifest contract test holds).

*Screens, walked live on the sandbox:* Program Settings → Monitoring & Testing (admin_monitoring.html: both lists, add/retire/delete, the retiring-never-erases note) and the chart's Testing Paradigm card on physician_detail (load/change/save inline, the licensing-board pattern; failure state shows with a retry — the S141 lesson). Sandbox demo config seeded through the real doors: STD24 paradigm + two fictional Seattle collection sites; Marcus Webb assigned live through the UI.

*Proof:* tests/insight/test_monitoring_core.cjs (101st test, 32 asserts, green after the oracle fix): CRUD + validations both directions, the full temporal assignment walk (assign → re-assign → end → history intact, one open row, audit rows written), referenced-delete refusals + clean delete, tenant confinement on lists AND deletes — run on the SANDBOX so nothing is Wisconsin-only. test_manifest_contract + test_tenant_standup_module re-run green (the new tables are filed; the copier carries paradigms). Lint 0. SERVER_VERSION 2026.08.03.0130, DB v150. Stories 2-5 queued: selection engine + calendar, check-ins + excused absences, toxicology results + safety-net wiring, the sandbox acceptance walk.

**Session 166 (2026-08-03, Bangalore hours) — ACCESS-RULES STORY 4: BREAK-GLASS AND THE IHS LOCKOUT (v149) — THE BUILD IS COMPLETE, AND THE ACCEPTANCE TEST WALKS ALL EIGHT BLOCKING CRITERIA.**

*The design pass first (Bill agreed before a line was written):* under rules mode, superuser logins ARE the spec's IHS Technical Staff column — no document content, no document metadata, through any door; the one path back in is an emergency grant the PROGRAM records (never the superuser), scoped to named documents, expiring in 24 hours, with automatic notification to the Medical Director and Program Administrator and a distinct audit event for every document opened. **The honest boundary, stated in the design, on the screen, and here:** this locks every interface the platform has; it cannot lock the database itself — the Heroku credentials are governed by commitment and the audit trail, not by code, and any claim made to a program says it that way.

*The lockout:* `sessionDocAccess` no longer waves superusers through under rules — they get the deny-by-default empty role set plus a superuser mark. The finder lists them nothing (or exactly their granted documents); every single-document door 404s without a grant (no oracle); upload, classify, replace, hold, release, and export all refuse; the chart-export documents section is empty for them (no X anywhere). Program staff's matrix access is untouched, and mode `'open'` tenants are untouched — **nothing changes anywhere until Erica's flip**, same as stories 1–3.

*The grant (v149):* `break_glass_grant` (integer link, the document pattern) + `break_glass_grant_document`. Recorded by a program officer — MD or PA under rules, tenant admin otherwise; a superuser attempting it gets its own refusal ("a program officer records the grant"). Grantee validated as an active superuser login; documents enumerated (never a blanket grant); reason and approval reference required; expiry = granted + 24h in Bill-epoch datetime blocks, **checked at read time — no timer to break; at hour 25 the door is simply shut**. Grant + document rows + the required audit event commit in one transaction through the strict writer — a grant the audit trail doesn't know about must not exist. A revoke door shuts it early. Under a live grant: view and download of the named documents ONLY — every open writes a DISTINCT strict audit event before content ('B' card view, 'G' download; a Part 2 download keeps its legally-distinct 'P' and still requires the linked consent); any edit refuses ("break-glass access is view and download only").

*The notification (spec §7.1 "automatic"):* grant creation fires BREAK_GLASS_GRANT through the standing notification rail; v149 seeds the rules on all three workforce tenants (MD by position — data, not code — and PA by admin role); notification_rule is a copied manifest part so future tenants inherit. Observed and accepted: a person holding both hats (ChrisB is admin AND Medical Director) receives it once per hat — standing platform behavior for overlapping rules.

*Also in v149:* `document.hold_reason` — the §7.2 legal-hold reason text deferred from story 2. Placing or releasing a hold under rules now REQUIRES a reason; it rides the same edit-audit diff as the flag.

*The screen:* Program Settings gained an **Emergency Access** card → `admin_break_glass.html` (session-only + login redirect, the S163 pattern): the standing grant record (active/expired/revoked, who, what, why, who approved, revoke button) and the record-a-grant form (grantee username as free text — deliberately no superuser directory door, the S129 confinement; document search-and-add chips; reason + approval required). The page states the honest boundary in its own explainer. Walked live on the sandbox: record → notified → revoke, all through the screen.

*THE ACCEPTANCE TEST — the story-4 exit (spec §9):* new `test_access_rules_acceptance.cjs` (100th test, **148 asserts**, green) demonstrates every Blocking criterion end to end **on the sandbox — a copied tenant, so the walk also proves none of this is Wisconsin-only**: AC-1 (no permission → empty list, 404 card/file/edit, server-side), AC-2 (cross-program 404 for every role including a granted superuser), AC-3 (the release gate: MD/CM-only, one-way, distinct event — the portal's future read), AC-4 (registrant marked at birth through the real public-registration door, chart never shows it, activation moves nothing and counts what waits, explicit promote with its distinct event, post-activation uploads unmarked), AC-5 (audit table renamed live → card/file/finder all refuse and serve nothing, heal on restore), AC-6 (Tier 2 never bulk-exports; each exported row writes X; the superuser export section is empty under rules), AC-7 (no document delete door EXISTS, even authenticated; hold is MD/PA with reason recorded), AC-8 (the full break-glass walk: lockout, self-grant refused, plain staff refused, bad grantee/document refused, grant → both officers' notification rows in the database, exactly-one-document finder, un-named and cross-program documents still 404, B/G audit events counted, simulated hour-25 expiry shuts the door, revoke shuts it early, double-revoke refuses, record visibility by role). test_document_access updated for story 4 (176 → **182 asserts**: the old "superuser always passes" step is now the lockout proof; holds carry reasons; one Claude-context assert moved to the MD). Manifest contract green with the two grant tables filed as operational-not-copied. Lint 0. One real bug caught by the test on first run: link_tank hands back BIGINT next_link as a STRING — the grant door now normalizes to a number at allocation (the documents list on a fresh grant read empty otherwise).

*What lifts the real-files gate:* the acceptance test IS the §9 demonstration, but the gate stays until Bill and Erica flip a tenant to rules and the note to Erica flags the two story-1 spec wrinkles + the honest raw-database boundary. SERVER_VERSION 2026.08.02.2129, DB v149. **The four-story access-rules build is code-complete.**

**Session 165 part 4 (2026-08-03) — THE CI TRUTH: Session 164's CI was never green, and two hotfixes made it genuinely green.**

The push of stories 1–3 surfaced it: S164's wrap had recorded the *pages-build* workflow run as "CI GREEN" — the real CI run for `29c2043` failed at migration replay and nobody saw. Cause: S164's v145 taught the tenant copier to carry `survey.display_order`, but CI rebuilds its database by replaying every migration from a v42 baseline with **current** code — so v139 (the sandbox standup) ran the copier against a schema six versions too old for the column. Invisible to any local run (the local schema is already current) — the local-suite-plus-CI double gate Bill ruled to keep in S164 exists for exactly this class. Hotfix 1: the copier's survey copy checks the column exists and carries it only then; v145's backfill converges a replayed environment to the same end state. Hotfix 2: with migrations finally passing, the S164 performance-profile drift guard ran on CI **for the first time ever** and went red — it compares the demo page against "live" Wisconsin weights, but CI's replayed database only carries factory defaults. The snapshot-vs-live comparison now enforces on real-data environments only (local dev, where tuning live weights reds the suite exactly as designed), with the reason stated in the test's own pass line. CI genuinely green at `cea41ae` (run 30773833707, verified by the CI workflow's own conclusion). Lessons recorded in ACTIVE_WORK; the S164 record corrected in STATE.

**Session 165 part 3 (2026-08-02) — ACCESS-RULES STORY 3: THE REGISTRANT BOUNDARY, PROMOTION AT ACTIVATION, AND THE PARTICIPANT RELEASE ACTION (v148).**

*The registrant boundary (spec §6.2, AC-4):* a document uploaded for a person who has not yet signed the monitoring agreement is **marked at birth** (`document.registrant_doc`) and excluded server-side from every member-scoped document query unless the caller is an administrative surface that explicitly asks (`include_registrant=1` — the Documents filing cabinet, which shows a Registrant badge). The chart never asks, so intake-era documents never appear on a clinical surface. Whether a person is pre-activation is the **vertical's fact**: a new `memberIsPreActivation` callback (intake.js owns the rule — an intake lifecycle status exists and is not Participant) so the platform file never names INTAKE_STATUS, the same pattern as the deactivation guard. A reactivated returner is a registrant again — their new documents ride the boundary until re-activation, which is the §6.2 intent. Verified by construction: documents feed no instrument, score, index, or registry calculation anywhere.

*Promotion is explicit, one document at a time, never automatic:* activation moves **nothing** — its response now counts what awaits review and says so in the success message. The new promote door clears the mark after a staff review (classify permission at the document's tier; refuses while the person is still pre-activation; refuses documents already on the chart; writes its own distinct audit event). The detail modal carries the registrant banner with the Promote action.

*The participant release mechanism (spec §5, decision D-3 — the portal reads it when built):* the new release door stamps who and when and writes the distinct release audit event. Medical Director and Case Manager only under rules mode; only Filed, person-linked, chart documents; one-way (a second release refuses). **Which types release is data, not code** — `release_types` rows seeded with lab reports and prescriber documentation per §5, so Erica's open question Q-1 is a one-row change later. A replaced (corrected) file deliberately does NOT inherit the release — new content re-reviews and re-releases — but keeps its registrant state. The modal shows Confidentiality (+ Part 2 mark) and the release row on Filed person documents.

*Proof:* test_document_access 141 → **176 asserts**, green first run — a true registrant created through the public registration door, the full lifecycle walked (upload → marked → invisible to the chart → activation moves nothing → promote → visible → release → double-release refused → replace drops the release), type eligibility and the MD/CM-only rule both directions. Both intake tests re-run green (the activation response grew a field). Lint 0. Cabinet + modal walked in the browser. SERVER_VERSION 2026.08.02.1815, DB v148.

**Session 165 part 2 (2026-08-02) — ACCESS-RULES STORY 2: AUDIT-BEFORE-SERVE, THE TIER-2 EXPORT EXCLUSION, AND THE 42 CFR PART 2 PLUMBING (v147).**

*Audit-before-serve (AC-5) is real:* the audit writer split into a core plus two faces — the forgiving `logAudit` every other subsystem keeps (audit failures never break their operation), and `logAuditStrict`, which refuses. The four document content doors (finder list, card view, file download, chart export) now write their audit event strictly and **before** any content moves: if the audit row cannot be written, the request errors and nothing is served — no metadata, no bytes, no listing. Proven the honest way in the test: rename the audit table live, watch card/file/finder all refuse and withhold content, restore, watch them heal.

*The Tier-2 bulk-export exclusion (AC-6):* the participant chart export gained a **Documents section — metadata only** (title, type, tier, dates, version), never file contents; only **Filed** documents. **Sensitive (Tier 2), Restricted (Tier 3), and unclassified rows are excluded absolutely — every mode, every role, superusers included** ("regardless of the exporting role" is her spec's own phrase). Under rules mode the matrix's X column additionally decides per tier (MD and PA export Standard-tier, PA alone exports org-level, CM exports nothing). Every exported row writes its own Export audit event ('X', distinct from download) through the strict writer before the file serves — that is how export scope is recorded. The Export Chart modal on the participant chart gained the Documents checkbox (walked live in the browser: modal renders, CSV request carries the section, 200).

*Part 2 plumbing (v147):* `document.part2_flag` + `document.part2_consent_link`. Until the consent architecture builds its own object, the consent artifact is a **Filed** document in the same program — and when both sides are person-linked, the **same person**. The rules ride the flag in every mode, law-shaped like the messaging consent gate: a flagged document refuses download without a live Filed consent linked (plain-English refusal); a permitted download writes the **distinct disclosure audit event ('P', never plain 'W')** and carries the redisclosure-prohibition notice on the response; replace carries both columns forward to the new version. No document is flagged today — plumbing now, water later, exactly the decision-register pattern.

*Deliberately not in this story:* legal-hold reason text (rides story 4 polish), participant portal visibility and release actions (story 3), break-glass (story 4). *Proof:* test_document_access grew 115 → **141 asserts**, green — Part 2 both directions with audit-row counts, the rename-the-audit-table AC-5 proof, MD-vs-CM export under rules, Tier-2 exclusion in both modes. Lint 0. SERVER_VERSION 2026.08.02.1733, DB v147.

**Session 165 (2026-08-02, Bangalore hours) — ACCESS-RULES BUILD STORY 1: THE TIER × ROLE PERMISSION MATRIX (v146). Erica's PI2_Document_Access_Rules spec is the contract; the S164-settled design was built without re-litigation.**

*What shipped (local):* Document access under mode `'rules'` is now computed from **role × confidentiality tier × lifecycle status — deny by default** (spec §1). The existing `document.confidentiality` column carries the spec §3 tier semantics (1=Standard, 2=Sensitive, 3=Restricted, 4=Org-level). **Migration v146** seeds `document_type.default_confidentiality` per spec §5 (license/contract→Org-level; consent+correspondence→Standard; fax/assessment/eval note/lab/prescriber/other→Sensitive) on all three workforce tenants with a cross-tenant parity check, backfills typed documents from their type defaults (0 rows needed changing — both live documents are consents, already Standard), and seeds the per-tenant **role map** (`sysparm document_access` / category `role_map`: MD→`position:POSITIONCLINIC:MEDDIR`, CM→`position:POSITIONCLINIC:CASEMAN`, PA→`admin`) — role vocabulary is DATA read through the v130 audience machinery, so the platform file still never names a vertical molecule.

*The matrix* (spec §4) lives as a code constant keyed tier×role → permission codes V/D/U/C/S/H/X. Multi-role sessions get the **union** of their roles' permissions (Erica holds both positions). Unclassified documents ride **Tier 2 until classified** (replaces v130's admin-only rule). Enforcement: `resolveDocumentTarget` requires V at the document's tier with the §6.1 lifecycle overlay (Superseded = MD/CM only; PA keeps superseded **org-level** documents — spec wrinkle (a), flagged for Erica) — invisible answers 404 exactly like a missing document, no oracle. Per-door codes: file→D (PA can view Tier 2 to classify but **not** download — a visible refusal is 403), edit/classify→C, replace→S, legal hold→H (MD+PA; admin-only rule stands under mode `'open'`), upload→U at the type's **default** tier, with unclassified uploads open to any mapped role (wrinkle (b), flagged for Erica). The finder's WHERE clause is built from the session's permitted tier×status combinations. Typed uploads are stamped at their type's default tier in every mode; classification stamps the new type's default; explicit tier changes: **raise = any classifier with C, lowering below the type default = Medical Director only**, all logged before/after through the existing edit audit.

*Deliberately unchanged:* mode `'open'` = pre-v146 behavior exactly — **Wisconsin live stays open until Erica's flip; nothing user-visible ships with this story.** `document_access_rule` rows are stored but not consulted (they stay as the home for future per-type overrides + story 3's participant rules). Audit-before-serve, Tier-2 export exclusion, Part 2 flag (story 2), registrant boundary + release actions (story 3), break-glass (story 4) — not in this story. The real-files gate STANDS until AC-1..AC-8 prove out at the end of story 4.

*Proof:* `test_document_access` REWRITTEN (33 → **115 asserts**, first run green): spec-default parity on all three tenants; open-mode unchanged; deny-by-default with no oracle; each role's row of the matrix walked (MD org view-only, CM org invisible, PA Tier-2 view-no-download, H restricted to MD+PA); wrinkle (b) both directions (PA refused a typed LAB upload, CM allowed); classification stamping; the tier-change rules (CM refused to lower, CM raise makes the document vanish from CM, MD lowers); the PA∪CM union login; superseded visibility incl. the org-level PA exception; the reversible flip back to open. Lint 0. SERVER_VERSION 2026.08.02.1710, DB v146.
...
**Session 162 (2026-08-01) — THE WISCONSIN-ASSUMPTIONS AUDIT: seven fixes shipped as they were found, and the audit document for the rest. The safety-detector layer works again — it had been quietly broken in three stacked ways.**

*The day's shape (Bill's direction, given twice: FIX as you find, don't catalog-and-rank):* the session opened on the audit's pre-confirmed customer and each fix's proof exposed the next bug beneath it. Seven fixes, six commits, all local (NOT pushed — full suite + CI + Bill's gos still owed): `7b6ab29` (fixes 1-4), `5f77f47` (fix 5), `d4bc4b9` (the audit doc), `bc7efcf` (fix 6), `d015032` (fix 7).

*Fix 1 — protective collapse fires on every tenant:* custauth filtered survey answers by wi_php's raw category numbers (4/6/7); wa_php's ISOLATION is −32765, the sandbox's −32740 — the safety detector matched ZERO rows on any copied tenant. Now joins survey_question_category by CODE. SQL parity proven on wi_php (178 rows, zero drift).

*Fix 2 — THE DEAD FILING PATH (the big one):* when the PPII band thresholds or the pattern detectors decided to file a registry item, they filed it by internal HTTP POST back into the accruals route — as an unauthenticated visitor. The S121-era auth wall (commit b67a01d, 2026-03-19) 401ed that call and the response was never read: **no threshold or pattern registry item was created on ANY tenant, including live wi_php, from 2026-03-19 to 2026-08-01.** The per-survey SR_* alerts ride a different direct path and kept flowing — which is why nobody noticed. Fix: the route's body factored into processAccrual() (the executeMedRun precedent — ONE pipeline, two callers); custauth receives a createAccrual capability in its hook context and calls it in-process. Refusals are LOUD.

*Fix 3:* the protective-collapse read gained the ms.link DESC same-day tiebreaker (backdated sittings pin start_ts to noon — ordering was disk-order roulette).

*Fix 4 — the resurrection stepped on a five-month-old landmine:* the pattern signals were never added to the PPII_SIGNALS recursion guard (S95), and the duplicate-item check that used to stop the second pass matches nothing since v67 files items under the ACTION code (SR_YELLOW) instead of the signal name. Result once the path was alive: re-detect → re-file → nested accrual, 18 deep, until the connection pool drained (proven live by the new standing-guard test). Guard completed.

*Fix 5 — never the same news twice, restored (Bill's "fixing, not pushing" correction):* new openItemExistsForSignal() — an open item matches by the signal name (promotion-era rows) OR any action code the tenant's ACTIVE bonuses file for that signal, resolved from live config, never a hardcoded map. A 4th worsening sitting while the item is open now files NOTHING (asserted).

*Fix 6 — spike and trend see again:* the PPII history read and the events-stream prior joined molecule_value_embedded_list — EMPTY on every tenant since the ~S126 internal-list era (nothing writes it) — so scores was always [] and PPII_SPIKE / PPII_TREND_UP could never fire ANYWHERE, Wisconsin included. The March 2026 registry items prove they once worked; this broke separately from fix 2 and the two failures masked each other. Both reads now match ACCRUAL_TYPE by the box-encoded byte (the Stream G pattern — S134 fixed one join, missed these two).

*Fix 7 — the participant app works on every tenant:* poser_mobile hardcoded SURVEY_LINK=1; on copied tenants the Weekly Check-in (the app's primary button) rendered an empty survey and the mini-PPSI + distress auto-expand gates could never match. Now resolves PPSI by code; proven live in the browser on the sandbox (34 questions load, mini set of 8, answer 2 auto-expands to full).

*THE STANDING GUARD:* test_pattern_triggers grew from 66 to 78 asserts — part 2 walks the whole protective-collapse chain on the SANDBOX tenant (fresh member built through real doors, same-noon sittings deliberately forcing the tiebreaker, exactly ONE item, 4th-sitting suppression); part 3 forces a real spike (0→~20) and a real trend (0→~7→~14) with all answers ≤2 so nothing else files. This test caught fixes 2, 4 and the severe-single-item watchdog doing its job — it earned its keep the day it was written.

*THE AUDIT DOCUMENT:* docs/WISCONSIN_ASSUMPTIONS_AUDIT_2026_08.md — five read-only lenses, every headline claim verified against the live DB before being written. REMAINING Tier 1 (the next session's fix queue, in order): 1.3 compliance_rules.html writes to Wisconsin by default (URL-param tenant, superuser-honored); 1.4 the weight-set admin screens default to wi_php/Delta and saves land there (+ unguarded weight GETs); 1.5 the ||5 fallback on 14 shared screens (poser_mobile's redirect is the blessed pattern, never propagated); 1.6 Central time hardcoded in custauth signal dates + notification-window defaults + program_tz (WA is Pacific); 1.7 the sandbox has ZERO document_type rows (verified 9/9/0 — the Aug 13 party hits this; needs a small migration + copier part). Tier 2/3 + six standing-guard recommendations (lint rules, census upgrades, a sandbox-tenant UI test run) are in the doc. Verified non-findings recorded so nobody re-chases them.

*Deploy note for the release that carries this:* live wi_php will genuinely START filing band/pattern alerts again after four and a half silent months — Erica may see items appear that "would have" filed long ago. That is the system working again, but the release note wording matters.

**Session 163 (2026-08-02, Bangalore hours) — THE AUDIT CLOSED OUT: Session 162's seven fixes DEPLOYED LIVE, then every remaining Tier 1, Tier 2, and Tier 3 item fixed in one session. Four of the six standing guards are now real. The test runner can never lose a failure's name again.**

*The deploy (first order of business, Bill's go):* the seven S162 audit fixes went to Heroku (2026.08.01.2031 / DB v142 at deploy time), live-verified (version/db match, dyno up, 401 probe). Wisconsin's band/pattern alert layer is filing again after the 4.5 silent months. Erica-activity window confirmed clear on Bill's word (she was not online). The release note conversation produced a calibration lesson recorded below.

*TIER 1, ALL FIVE (commit `a554fca`, deployed same session):* (1.3) compliance_rules.html session-only + login redirect (was URL-param tenant defaulting '5' — a superuser from a WA context read AND edited Wisconsin's live compliance items). (1.4) both weight-admin screens session-only + redirect (fallbacks were '5' and '1' — Delta); scoring_admin.js weight GETs gained canReadTenantWeights (own tenant or superuser — any authed user could read any tenant's clinical weights before). (1.5) the poser_mobile session+redirect pattern propagated to all 13 remaining shared clinical screens. (1.6) timezone is tenant data now: custauth signal accruals stamp activity_date in the tenant's timezone from notification_delivery_config; buckets program_tz reads the real config; the copier inherits the SOURCE tenant's timezone; one PLATFORM_DEFAULT_TZ constant for the two true no-row defaults. (1.7) v142 backfilled the sandbox's 9 document types from wi_php (count-verified) — the Aug 13 orientation blocker; copier + manifest now carry document_type AND document_access_rule.

*TIER 2 (commits `94d4206`, `9b9d803`):* (2.1) reward references remap by their OWN type through one mapResultReference (external→action, tier→tier_definition, token→adjustment; loud on ANY miss — silent nulling was a reward vanishing without a trace); legacy required_tier_id/reward_tier_id remap too; tier_definition + adjustment copy with maps. BADGES RULED CONTENT, NOT CONFIG: wi_php's six badges are AFFILIATIONS (Wisconsin Medical Society, UW Health...) — state content like partners; never copied, recorded in the not-copied list, and a badge-awarding result refuses to copy with wire-it-deliberately instructions (the enroll-chain precedent). (2.2) manifest gaps closed: molecule_group/_member + the three non-text static value tables copy inside the molecule loop; redemption_rule/_point_type + alias_composite/_detail copy with remapped references; survey_question_category/_list finally COUNTED; ~10 new manifest parts (tenant-less ones via parent-join countSql); partner/partner_program, evaluator, network_entity/program_network_entry, badge all recorded not-copied with reasons — the header contract holds again. v143 backfilled wi_php's one molecule group (MIDWEST states, used by no rule) to BOTH Washington tenants for gate parity. (2.3) encodeValue REFUSES negatives in offset/squish regimes in plain English — the v140 class becomes a first-write refusal, not a bare 500; dead updateMoleculeRowByTable deleted. (2.4) offset-regime census v2: any value_kind, columns 2..N, un-probe-able rows NAMED (17 today) instead of silently dropped. (2.5) Wisconsin left the seeders: seed_physicians + seed_pulse_events resolve surveys by code and take tenant arguments; ml_report takes tenant from the command line and names it; the 10M data-loader builds tier patterns from the target tenant's own ladder (was Delta ids 2/3/4/7).

*TIER 3 (commit `fe0a4b2`, migration v144):* startPPSI DELETED from the platform-shared modal (zero callers; the lint itself rejected the delegate form because it named PPSI in a root file — the rule proved itself); getMemberBadgeOnDate decodes via decodeValue (was a second hand-rolled +32768); truthiness-as-link-existence fixed at the three named sites (0 is a legal link); poser_mobile no longer greets contextless visitors as a fabricated "Dr. James Mitchell"; affiliations placeholder de-Wisconsinned; v144 moved the platform debug flag to tenant 0 (strays under tenants 1/3 removed). DELIBERATELY UNTOUCHED: performance_profile's hardcoded snapshot (Bill's call, unchanged); meds.js/instruments.js ORDER BY link (a portable sort needs a display_order column — schema decision for Bill); the historical one-time SQL script's COALESCE.

*THE TEST RUNNER FIX (commit `2e51847`, Bill's direct order after a failure's name was lost):* tests/run.cjs appends every test's result to tests/last_run.log AS IT FINISHES (name, counts, each failing assertion's text; gitignored; overwritten per run) — a truncated screen can never lose the failing test's identity again. Born from a real loss: run 1 of the day had one test fail (3 asserts), the tail-piped output ate the name, and the whole 10-minute suite ran again just to learn it. The blip never recurred across three subsequent full runs.

*SESSION LESSON (Bill's calibration, recorded because it changed how this session reported):* nothing is everything, and everything is nothing — the S162 story had been narrated in five-alarm language ("all hell", "4.5 months of silence") and then collapsed to "status quo, nobody affected" under Bill's questioning. Both were me describing the same facts. The rule now: every finding is reported at one consistent size, set by exactly two questions — who is affected, and what action does anyone need to take. "Nobody" and "none" = a line item, not a headline. The platform is under test; bugs found and fixed are the process working, not incidents.

*Standing guards status:* #3 (census v2) and #4 (encode sign guard) BUILT this session. Remaining for a next session: #1 (lint: numeric literal into *_link/*_id names), #2 (lint: tenant fallback || literal in client pages), #5 (one parameterized UI-test run of the daily clinical screens against the SANDBOX tenant), #6 (manifest contract check — machine-diff REQUIRED_PARTS + not-copied list against the schema; needs the not-copied list made machine-readable first).

**Session 164 (2026-08-02, Bangalore hours) — THE FIXING ERA CLOSED: the last four standing guards built, both parked decisions fixed, the demo-side tenant-fallback family swept — and the ACCESS-RULES BUILD opened with Bill's go on Story 1.**

*THE FOUR STANDING GUARDS (commits `290053e`, `0cd90a3`, `e3637b7`):* Lint Pattern 10 fails the build on any numeric literal assigned/compared to a `*_link`/`*_id` name — the whole S160-162 Wisconsin-id family (SURVEY_LINK = 1, survey_link === 1, category_link IN (4,6,7)), each proven caught by planted probe before the probes were removed; 0-sentinels, db_migrate, tests, and six filename-declares-its-tenant one-time seeds exempt. Lint Pattern 11 fails tenant-fallback literals (`|| 5`, `|| '1'`) — born scoped to the clinical surface, widened to the WHOLE codebase by end of session (see the sweep). Building the rules found two `req.tenantId || '1'` stragglers the S147 audit-1.2 sweep had missed (/v1/tiers, /v1/molecules/by-source) — both now answer the same plain-English 400 as the other 52 routes. The manifest contract test (98th test): tenant_standup's deliberately-not-copied prose became the machine-readable NOT_COPIED export, and the test censuses all 140 schema tables — every table must be in REQUIRED_PARTS (directly or via countSql), NOT_COPIED, a vertical's PARTS, or the test's explicit COPIED_WITH/OPERATIONAL/PLATFORM lists; an unfiled table fails the suite with filing instructions (the survey_question_answer silence is structurally impossible now). Building it found compliance_item_status — every compliance item's status codes/scores/sentinel flags — copied since the module was born but never COUNTED; now a countSql manifest part (24/24/24 across the three workforce tenants). And guard #5: test_insight_page_geometry is now a parameterized TWO-TENANT sweep (115 asserts, was 54) — Erica's seven screens + seven pinned modal bars measured on Wisconsin AND the sandbox, the sandbox run building its own persona through real doors (enroll → intake notes → clinic assignment via the member molecule-rows door → PPSI by code → three sittings, the third elevated so a REAL registry item files through the S162-revived detector chain). Learned en route: the staff enroll door stamps PARTICIPANT immediately (Phase-1 semantics), so the activation door rightly 409s an enroll-door person.

*THE TWO PARKED DECISIONS (Bill: "fix it and fix the other two"):* (1) v145 `survey.display_order` (commit `8f7f6fb`) — the instrument catalog sorted ORDER BY link, catalog order only on Wisconsin by legacy-link luck; Chris's team would have seen a shuffled catalog Aug 13. wi_php's link order backfills as the reference, every tenant matches BY CODE (migration refuses on drift), readers order by it, the copier carries it, the create door assigns max+1. Verified identical across all three workforce tenants. (2) performance_profile drift guard (commit `eb83eb3`) — the public no-login demo page keeps its baked-in weights (a public endpoint for live clinical config would walk back S163's own hardening) but can no longer lie: a standing test diffs the page's PPSI_WEIGHTS/PPSI_TIERS against the live wi_php weight set + ppii_thresholds and names exactly what to update on drift (red path proven by perturbing one digit).

*THE DEMO-SIDE SWEEP (commit `29c2043`, Bill's ruling after the lint work surfaced it):* 105 `|| '1'` (Delta) fallbacks across 51 root pages/modules — the same disease S163 cured on the clinical screens, demo-contained until today. Every page now reads the session only and bounces a sessionless tab to login (verified live in a fresh browser); shared modules lost their fallbacks (pages own navigation); root admin_settings.html stopped taking its tenant from the URL; stress_client refuses to run without an explicit config.tenantId and names its tenant (the ml_report precedent). Pattern 11 then widened codebase-wide: 0 matches. The bug shape is extinct and unwritable everywhere.
