...
const SERVER_VERSION = "2025.11.08.2120"; // Format: YYYY.MM.DD.HHMM
const BUILD_NOTES = "Fixed redemption_rule endpoints to use ACTUAL column names from database: redemption_id (PK), redemption_description (not rule_id or redemption_desc)";
...
app.get("/v1/member/:id/activities", async (req, res) => {
  ...
  try {
    ...
    // Step 3.5: Load BOTH Efficient and Verbose display templates
    ...
    try {
      const efficientQuery = `
        ...
        WHERE dt.tenant_id = $1 AND dt.template_type = 'E'
        ORDER BY dtl.line_number
      `;
      const efficientResult = await dbClient.query(efficientQuery, [tenantId]);
      if (efficientResult.rows.length > 0) {
        efficientTemplate = efficientResult.rows.map(row => row.template_string);
        console.log(`Loaded Efficient display template with ${efficientTemplate.length} lines`);
      }
    } catch (e) {
      console.log('No Efficient template found');
    }
    ...
    try {
      const verboseQuery = `
        ...
        FROM display_template dt
        JOIN display_template_line dtl ON dt.template_id = dtl.template_id
        WHERE dt.tenant_id = $1 AND dt.template_type = 'V'
        ORDER BY dtl.line_number
      `;
      const verboseResult = await dbClient.query(verboseQuery, [tenantId]);
      if (verboseResult.rows.length > 0) {
        verboseTemplate = verboseResult.rows.map(row => row.template_string);
        console.log(`Loaded Verbose display template with ${verboseTemplate.length} lines`);
      }
    } catch (e) {
      console.log('No Verbose template found');
    }
    ...
    const activities = await Promise.all(activitiesResult.rows.map(async (activity) => {
      ...
      /* TODO: Uncomment when redemption_rule_id column is added to activity table
      // Special handling for redemptions (activity_type='R')
      if (activity.activity_type === 'R' && activity.redemption_rule_id) {
        try {
          const redemptionQuery = `
            SELECT redemption_code, redemption_description
            FROM redemption_rule
            WHERE redemption_id = $1 AND tenant_id = $2
          `;
          const redemptionResult = await dbClient.query(redemptionQuery, [activity.redemption_rule_id, tenantId]);
          
          if (redemptionResult.rows.length > 0) {
            const redemption = redemptionResult.rows[0];
            
            // Build redemption magic_box
            const redemptionMagicBox = [
              { label: 'Code', value: redemption.redemption_code },
              { label: 'Description', value: redemption.redemption_description }
            ];
            
            // Override all magic_box versions for redemptions
            result.magic_box = redemptionMagicBox;
            result.magic_box_efficient = redemptionMagicBox;
            result.magic_box_verbose = redemptionMagicBox;
            
            // Add to result for reference
            result.redemption_code = redemption.redemption_code;
            result.redemption_description = redemption.redemption_description;
          }
        } catch (error) {
...
app.post('/v1/display-templates', async (req, res) => {
  ...
  try {
    const { tenant_id, template_name, template_type, lines } = req.body;
    ...
    const templateQuery = `
      ...
      VALUES ($1, $2, $3, false)
      RETURNING template_id, template_name, template_type, is_active, created_at
    `;
...
app.put('/v1/display-templates/:id', async (req, res) => {
  ...
  try {
    const templateId = parseInt(req.params.id);
    const { template_name, template_type, lines } = req.body;
    ...
    const templateQuery = `
      ...
      SET template_name = $1, template_type = $2
      WHERE template_id = $3
      RETURNING template_id, template_name, template_type, is_active
    `;
