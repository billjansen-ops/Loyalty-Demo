...
echo
echo "3) Ensure demo tenant/member + one activity (molecule keys in activity_detail)…"
TENANT_ID=$(psql -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -X -A -t -c \
  "select tenant_id from public.tenant order by tenant_id limit 1;")
if [ -z "$TENANT_ID" ]; then
  TENANT_ID=$(psql -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -X -A -t -c \
    "insert into public.tenant(tenant_key,name,industry) values('demo','Demo Tenant','airline') returning tenant_id;")
...
MEMBER_ID=$(psql -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -X -A -t -c \
  "select member_id from public.member where tenant_id=${TENANT_ID} order by member_id limit 1;")
if [ -z "$MEMBER_ID" ]; then
  MEMBER_ID=$(psql -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -X -A -t -c \
    "insert into public.member(member_id,tenant_id,name) values(100001, ${TENANT_ID}, 'Bill Demo') returning member_id;")
...
HAS_ACTIVITY=$(psql -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -X -A -t -c \
  "select exists(select 1 from public.activity where member_id=${MEMBER_ID});")
if [ "$HAS_ACTIVITY" != "t" ]; then
  AID=$(psql -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -X -A -t -c \
    "insert into public.activity(member_id,activity_date,kind,subtype,point_amount,point_type)
  ...
  psql -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -X -q -c \
    "insert into public.activity_detail(activity_id,k,v_text) values
      (${AID}, 'carrier_code','DL'),
      (${AID}, 'origin','MSP'),
      (${AID}, 'destination','ATL'),
      (${AID}, 'fare_class','Y');"
