...
const SERVER_VERSION = "2025.12.15.1058";
const BUILD_NOTES = "Added counter_molecule_label to promotions endpoint for proper goal display";
 ...
 */
async function insertMoleculeRow(pLink, moleculeKey, values, tenantId) {
  ...
  // Determine attaches_to from context
  const attachesTo = info.context === 'activity' ? 'A' : 'M';
  ...
  const result = await dbClient.query(sql, encodedValues);
 ...
 * @returns {Promise<void>}
 */
async function deleteAllMoleculeRowsForLink(pLink, context) {
  const attachesTo = context === 'activity' ? 'A' : 'M';
  const tables = [
    '"5_data_1"',
    '"5_data_2"',
    '"5_data_3"',
  ...
  for (const table of tables) {
    try {
      await dbClient.query(`DELETE FROM ${table} WHERE p_link = $1 AND attaches_to = $2`, [pLink, attachesTo]);
    } catch (e) {
...
async function getNextLink(tenantId, tableKey) {
  ...
  if (result.rows.length === 0) {
    ...
    // Insert new row, start at 2 (we'll return 1)
    try {
      ...
      // Return 1 as first link
      if (linkBytes === 8) return 1;
      if (linkBytes === 2 || linkBytes === 4) return 1;
...
async function getSysparmValue(tenantId, keyOrCategory, categoryOrCode = null, codeOrClient = null, client = null) {
  ...
  console.log(`⚠️ SYSPARM CACHE MISS: ${cacheKey} (cache size: ${caches.sysparm.size})`);
  ...
  try {
    ...
    await db.query(
      `INSERT INTO sysparm_detail (sysparm_id, category, code, value)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (sysparm_id, category, code) WHERE category IS NOT NULL AND code IS NOT NULL
...
async function getAllActivityMolecules(activityId, tenantId, link = null) {
  ...
  for (const row of detailsResult.rows) {
    // For link types, pass raw value to decodeMolecule
    // For others, decode using storage rules first
    let decodedValue;
    if (row.value_type === 'link') {
      decodedValue = row.raw_value; // Raw bytes for link lookup
    } else {
      decodedValue = decodeValue(row.raw_value, row.storage_size, row.value_type);
    }
...
async function loadCompositeToCache(tenantId, compositeType, client) {
  ...
  console.log(`[CACHE] Reloaded composite ${cacheKey}`);
}
...
app.get('/v1/sysparm/:category/:code', async (req, res) => {
  ...
  try {
    ...
    // For activity type codes (A, P, J, R, M, N), check activity_processing sysparm
    const activityTypes = ['A', 'P', 'J', 'R', 'M', 'N'];
    let value;
    ...
    for (const field of template.fields) {
      ...
      if (molDefResult.rows.length > 0) {
        ...
        try {
          ...
          console.log(`No value for ${molKey}: ${e.message}`);
        }
      }
    }
...
app.post('/v1/member', async (req, res) => {
  ...
  try {
    ...
    console.log(`New member enrolled: ${newMember.membership_number} - ${fname} ${lname} (link: ${link})`);
  ...
  try {
    ...
    for (const criterion of criteriaResult.rows) {
      ...
      if (isLookupMolecule(moleculeDef)) {
        ...
        // For test-rule, we need member link - resolve from membership_number passed in body
        ...
        if (!memberRec) {
          console.log(`   ⚠ Member not found: ${memberIdentifier}`);
          failures.push(`${criterion.label} - Failed (member not found)`);
          continue;
        }
        
        const refContext = { member_link: memberRec.link };
...
app.post('/v1/test-promotion-rule/:promotionCode', async (req, res) => {
  ...
  try {
    ...
    if (isRestricted) {
      ...
      if (!memberLink) {
        console.log(`   ❌ FAIL: No member provided for restricted promotion check`);
    ...
    for (const criterion of criteriaResult.rows) {
      ...
      if (isLookupMolecule(moleculeDef)) {
        ...
        if (!memberLink) {
          console.log(`   ⚠ No member provided for reference molecule - skipping`);
          failures.push(`${criterion.label} - Failed (no member context)`);
          continue;
        }
...
app.post('/v1/activities/:activityLink/apply-bonus/:bonusCode', async (req, res) => {
  ...
  try {
    ...
    console.log(`   Activity: ${activityLink}, Bonus: ${bonusCode}`);
...
async function loadDebugSetting() {
  ...
  try {
    ...
    console.log(`🐛 Debug logging: ${DEBUG_ENABLED ? 'ENABLED' : 'DISABLED'}`);
  } catch (error) {
...
app.post('/v1/members/:memberId/activities/partner', async (req, res) => {
  ...
  try {
    ...
    console.log(`\n📝 Creating partner activity for member link ${memberLink}:`, {
      activity_date,
      partner_id,
      program_id,
      point_amount
    ...
    console.log(`   ✓ Created activity ${activityLink}`);
    ...
    console.log(`   ✓ Added member_points molecule (bucket: ${bucketResult.bucket_link}, amount: ${point_amount})`);
...
app.post('/v1/members/:memberId/activities/adjustment', async (req, res) => {
  ...
  try {
    ...
    console.log(`\n📝 Creating adjustment for member link ${memberLink}:`, {
      activity_date,
      adjustment_id,
      point_amount
    });
    ...
    // Evaluate promotions
    console.log(`\n🎯 Evaluating promotions for adjustment activity ${activityLink}...`);
    const promotions = await evaluatePromotions(null, activity_date, memberLink, tenantId, activityLink);
...
app.put('/v1/system/sync-commit', async (req, res) => {
  ...
  try {
    ...
    console.log(`💾 Synchronous commit ${enabled ? 'ENABLED (safe)' : 'DISABLED (fast)'} for database ${targetDb}`);
...
app.post('/v1/admin/clear-member-data', async (req, res) => {
  ...
  try {
    ...
    for (const del of deletions) {
      ...
      console.log(`   ✓ Deleted ${result.rowCount} rows from ${del.table}`);
    }
    ...
    console.log(`   ✓ Complete! Members preserved: ${memberCount.rows[0].count}`);
...
app.post('/v1/admin/stress-test/start', async (req, res) => {
  try {
    ...
    console.log(`   Date Range: ${config.dateRangeMonths || 24} months`);
    ...
    const workerFn = async (workerId) => {
      while (job.status === 'running') {
        ...
        try {
          ...
          // Resolve to internal member link (this is what the API does)
          const memberRec = await resolveMember(membershipNumber, tenantId);
          if (!memberRec) {
          ...
          // Random date within configured range (default 24 months)
          const dateRangeMonths = config.dateRangeMonths || 24;
          const maxDaysAgo = dateRangeMonths * 30;  // Approximate
          ...
          const activityData = {
            ...
            base_points: randomInt(config.pointsMin || 500, config.pointsMax || 1500)
          };
          
          // Call the same function the API uses (unless dry run)
          if (!config.dryRun) {
            await createAccrualActivity(memberLink, activityData, tenantId);
          }
...
app.post('/v1/admin/api-stress-test/start', async (req, res) => {
  try {
    ...
    console.log(`   Requests: ${config.requestCount.toLocaleString()}`);
    console.log(`   Concurrency: ${config.concurrency}`);
...
async function runApiStressTestJob(jobId) {
  ...
  try {
    ...
    console.log(`   Rate: ${rate} req/sec, Avg response: ${avgResponse}ms`);
...
app.listen(PORT, async () => {
  ...
  console.log(`Database: ${dbClient ? `CONNECTED to ${currentDatabaseName}` : 'NOT CONNECTED - using mock data'}`);
  console.log(`Sync Commit: ${syncStatus} (${syncStatus === 'off' ? 'fast' : 'safe'})`);
  console.log(`===========================================\n`);
});
