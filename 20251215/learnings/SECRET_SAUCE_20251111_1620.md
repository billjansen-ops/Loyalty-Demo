# SECRET SAUCE
**Updated:** November 11, 2025 16:20
**Version:** 20251111_1620
...
├── Which expiration rule applies
├── Which tier qualification rules apply
├── Which bonus rules are active
└── When points expire
...
┌─────────────┬─────────────┬──────────┐
│ activity_id │ molecule_id │ v_ref_id │  ← 12 bytes per detail
├─────────────┼─────────────┼──────────┤
│     1001    │      7      │    42    │  Instead of "Delta Air Lines"
│     1001    │      8      │   125    │  Instead of "Minneapolis"
│     1001    │      9      │    88    │  Instead of "Boston"
└─────────────┴─────────────┴──────────┘
```
...
┌──────────────────────────────────────┐
│         ACTIVITY (Universal)         │
│  activity_id, member_id, date, type  │
└────────────┬─────────────────────────┘
             │
        ┌────▼──────────────────────────────────────┐
        │  ACTIVITY_DETAIL (Pointers to molecules)  │
        │  activity_id, molecule_id, v_ref_id       │
        └──────┬────────────────────────────────────┘
               │
    ┌──────────▼──────────┐
    │  MOLECULE_DEF       │
    │  "carrier"          │────► molecule_value_text: "Delta", "United", ...
    │  "hotel_brand"      │────► molecule_value_text: "Marriott", "Hilton", ...
    │  "rental_company"   │────► molecule_value_text: "Hertz", "Avis", ...
    │  "redemption"       │────► molecule_value_lookup: redemption_rule table
    └─────────────────────┘
```
...
**Wrong approach:**
```javascript
const icon = activity_type === 'R' ? '🎁' : '✈️';  // Hardcoded
```
...
## 🔍 CSR Bonus Testing & Debugging (GAME CHANGER)
**Added:** November 11, 2025
**Status:** This is a major competitive differentiator - possibly unique in the industry
...
│                                                      │
│  Flight: DL 123, JFK → LAX, 11/11/2025             │
│  Fare Class: Y (Economy)                            │
│  Miles Earned: 2,500                                │
│                                                      │
│  Bonuses Applied: 0                                 │
│                                                      │
│  [🧪 Test Bonus]  ← CSR clicks this                │
└─────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────┐
│  Test Bonus Rule                                    │
│                                                      │
│  Select Bonus: [First Class 50% Bonus      ▼]      │
│                                                      │
│  [Test Rule!]                                       │
│                                                      │
│  ❌ FAIL                                            │
│  Reason: Fly First: Failed                          │
│  Expected: F (First Class)                          │
│  Actual: Y (Economy)                                │
│                                                      │
│  [Close]                                            │
└─────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────┐
│  ✅ PASS                                            │
│  This activity qualifies for the bonus!             │
│                                                      │
│  [Apply Bonus to Activity]  ← CSR can fix it!      │
└─────────────────────────────────────────────────────┘
```

### How It Works (Technical)

**1. Test Any Bonus Against Any Activity**
```javascript
// CSR selects activity + bonus to test
POST /v1/test-rule/{bonusCode}
{
  "activity_date": "2025-11-11",
  "carrier": "DL",
  "origin": "JFK", 
  "destination": "LAX",
  "fare_class": "Y",
...
{
  "pass": false,
  "reason": "Fly First: Failed (expected F, got Y)"
}
```

**2. Shared Criteria Evaluation Logic**
- Test endpoint uses SAME evaluation code as bonus engine
- No divergence between testing and production
- If test passes, production would pass (and vice versa)

**3. Manual Application (With Audit Trail)**
```javascript
// If CSR determines bonus should apply
POST /v1/activities/{activityId}/apply-bonus/{bonusCode}

// Uses SAME function as automatic application
...
// Result:
// - Creates activity_bonus record
// - Updates point_lot with bonus points
...
**1. Instant Diagnosis**
```
Before: "Let me escalate this to IT..." (hours/days)
After:  "Let me test that... shows you didn't fly First Class" (30 seconds)
```

**2. Empowered CSRs**
- Can answer "why" questions immediately
- Can fix legitimate issues on the spot
- No developer involvement needed
- Member leaves satisfied

**3. Handles Complex Scenarios**
```
Member: "I should have gotten the Europe double miles bonus!"
CSR: [tests EUROPE_DOUBLE bonus]
Result: ❌ FAIL - Origin: Expected EUR zone, got JFK (US)
CSR: "Your flight originated in the US, so it doesn't qualify"
```

**4. Edge Cases & Overrides**
```
Member: "I flew First but the system coded it as Economy"
CSR: [checks booking, sees documentation]
CSR: [tests FIRST_CLASS bonus]
Result: ❌ FAIL - but CSR has proof
CSR: [clicks Apply Bonus]
Result: ✅ Manual adjustment with audit trail
```

**5. Learning Tool**
- New CSRs learn bonus rules by testing
- See exactly which criteria matter
- Understand why bonuses do/don't apply
- No need to read 500-page policy manual

### Architectural Innovation

**Key Insight: Make the bonus engine testable**

Traditional systems:
```
evaluateBonuses(activity) {
  // 500 lines of logic
  // Calculation + criteria checking all mixed together
  // Can't test individual rules
  // Can't reuse for manual application
}
```
...
async function applyBonusToActivity(
  activityId, bonusId, bonusCode, 
  bonusType, bonusAmount, basePoints
) {
...
// Automatic (during activity creation)
evaluateBonuses() {
  for (bonus of allBonuses) {
    if (criteriaPass(bonus, activity)) {
      await applyBonusToActivity(...);  ← Shared!
    }
  }
}

// Manual (CSR testing/override)
POST /apply-bonus {
  // CSR clicked "Apply Bonus"
  await applyBonusToActivity(...);  ← Same function!
}
```

**Benefits:**
- Test uses SAME logic as production
- Manual application is identical to automatic
- One function to maintain, not two
- Impossible for test and production to diverge

### Competitive Advantage

**Most loyalty systems CANNOT do this because:**

1. **Batch Processing** - Bonuses calculated overnight, can't test on-demand
2. **Monolithic Code** - Business logic buried in code, can't expose to UI
3. **No Separation** - Criteria evaluation mixed with application logic
4. **Audit Trail** - Manual adjustments tracked separately (or not at all)

**This system CAN because:**

1. **Real-time** - Everything on-demand, test instantly
2. **Data-Driven** - Rules in database, accessible to test UI
3. **Clean Separation** - Test criteria separately from applying bonus
4. **Unified Code** - Manual = Automatic, same audit trail

### Real-World Impact

**Scenario: Status Match Challenge**
```
Promotion: "Fly 3 qualifying flights by Dec 31, get 5,000 bonus miles"

Member calls after 3rd flight:
CSR: [tests STATUS_MATCH_CHALLENGE bonus]
Result: ✅ PASS
CSR: "I can see you qualify! Let me apply that bonus..."
CSR: [clicks Apply Bonus]
Member: "Wow, that was fast!"

Alternative (member calls before 3rd flight):
CSR: [tests STATUS_MATCH_CHALLENGE bonus]  
Result: ❌ FAIL - Reason: Qualifying flights: Expected 3, got 2
CSR: "You have 2 of 3 flights. After your next flight, it'll apply automatically!"
```

**No escalation. No tickets. No waiting.**
...
1. **"What If" Testing**
   - "If I fly to London next week, which bonuses apply?"
   - Test hypothetical activities before booking

2. **Bulk Testing**
   - "Why didn't these 50 members get the promotion?"
   - Test entire segment, see patterns

3. **Promotion Preview**
   - "Show me which of my upcoming flights will earn this new bonus"
   - Test against member's future bookings

4. **Historical Analysis**
   - "If we had this bonus active last quarter, how many members would have qualified?"
   - Test retroactive scenarios

### The Bottom Line

**This isn't just a debugging tool - it's a paradigm shift.**

Traditional: Loyalty platforms are black boxes. Members and CSRs are at the mercy of batch processing.

This system: Loyalty platforms are transparent. Test anything, see why, fix issues instantly.

**When you have hundreds of promotions:**
- Testing capability isn't nice-to-have
- It's essential to operations
- It's the difference between happy and frustrated members
- It's what separates modern systems from legacy systems

**The secret sauce:** Making the invisible visible. Making the automatic testable. Making the complex simple.

---

## 🔬 The Molecule System (Deep Dive)
...
├── molecule_id (PK)
├── tenant_id
├── molecule_key ('carrier', 'hotel_brand', 'activity_display', etc.)
├── label ('Carrier', 'Hotel Brand', 'Activity Display Config')
├── value_kind ('text', 'scalar', 'list', 'embedded_list', 'lookup')
└── context ('activity', 'member', 'program')

Child Tables (Values):
├── molecule_value_text          # Text values ("Delta", "United")
├── molecule_value_scalar        # Single values ("Miles", "90")
├── molecule_value_list          # Lists with code/label pairs
├── molecule_value_embedded_list # Categorized multi-property lists
└── molecule_value_lookup        # External table references

Transactional Usage:
└── activity_detail              # Points to molecule values
    ├── activity_id
    ├── molecule_id ────────────► molecule_def
    └── v_ref_id ───────────────► molecule_value_* (row ID or external ID)
```
...
embedded_value_id │ molecule_id │ tenant_id │ category │ code      │ description
──────────────────┼─────────────┼───────────┼──────────┼───────────┼─────────────
        1         │     10      │     1     │   'A'    │ 'icon'    │ '✈️'
        2         │     10      │     1     │   'A'    │ 'color'   │ '#059669'
        3         │     10      │     1     │   'A'    │ 'bg_color'│ '#f0fdf4'
        4         │     10      │     1     │   'R'    │ 'icon'    │ '🎁'
        5         │     10      │     1     │   'R'    │ 'color'   │ '#dc2626'
...
lookup_id │ molecule_id │ table_name       │ id_column      │ code_column       │ label_column
──────────┼─────────────┼──────────────────┼────────────────┼───────────────────┼──────────────────────
    1     │      5      │ carriers         │ carrier_id     │ code              │ name
    2     │     12      │ redemption_rule  │ redemption_id  │ redemption_code   │ redemption_description
...
INSERT INTO molecule_value_embedded_list VALUES
  ...
  (molecule_id, tenant_id, 'X', 'icon', '⚙️', 2),
...
-- UI automatically shows blue ⚙️ icon
...
INSERT INTO bonus_rule VALUES (
  'CARRIER_DL_BONUS', 
  'carrier = DL',      ← molecule-based condition
  1.5
);
...
INSERT INTO bonus_rule VALUES (
  ...
  'hotel_brand = MARRIOTT',  ← same pattern!
  2.0
);
```
...
- Average flight activity: ~8 molecules × 12 bytes = 96 bytes
- Same data with text: ~200+ bytes
- 50% space savings = 2× more fits in cache
- Integer joins are 10× faster than string joins
...
## 🕰️ Historical Context - Bill's "Million Years Ago" Philosophy
...
Pointer-based:
- Follow pointers once (activity → molecule → value)
...
## 💡 Key Discoveries (Session by Session)
...
- Change currency from "miles" to "kilometers" → all errors update
...
**What we tried:**
1. `grid-template-columns: 1fr auto` → 10+ inches of whitespace
2. `grid-template-columns: auto auto` → still huge whitespace
3. `margin-left: 20px` → worked perfectly
...
### Discovery 7: CSR Bonus Testing - The Game Changer (November 11, 2025) 🔥
**Problem:** Airlines have HUNDREDS of promotions. CSRs can't diagnose why bonuses didn't apply. Members frustrated. Developers spend hours debugging individual cases.

**The Breakthrough:** Make the bonus engine testable AND manually appliable.

**What we built:**
1. **Test Any Bonus Against Any Activity**
   - CSR selects activity + bonus
   - System runs SAME criteria evaluation as production
   - Shows exact reason for pass/fail
   - 30 seconds instead of hours

2. **Manual Application With Audit Trail**
   - If test passes but bonus wasn't applied (edge case)
   - CSR can apply bonus on the spot
   - Uses SAME function as automatic application
   - Full audit trail
...
## 🔮 Future Innovations (Ideas for Next Sessions)
...
## 🎯 The Molecule Philosophy (Summary)
