...
- ✅ **Dynamic text uses atoms**: ALWAYS use `resolveAtom()` or `resolveAtoms()` for any text that includes labels, field values, or dynamic content
  - Error messages: "Member does not have enough {{M,point_type,label,,L}}"
  - Display text: "Welcome back, {{T,members,first_name,,P}}!"
  - Templates: Any string that needs molecule labels or database values
- ❌ NEVER hardcode "miles", "Flight", "Origin", "Economy", etc.
- ❌ NEVER query molecules or tables directly for display text (use atoms instead)
- ❌ NEVER use `SELECT label FROM molecule_def WHERE...` when `resolveAtom()` exists

**Why:** System must work for any industry without code changes. Atoms provide a consistent abstraction layer for all dynamic text.
...
## 🧪 Atoms - Dynamic Text Resolution

### What Are Atoms?

Atoms are template variables for dynamic text that get resolved from molecules or database tables. They provide a consistent abstraction layer for all labels, field values, and dynamic content in the system.

**Key Concept:** Never hardcode display text. Always use atoms.
...
**Parameters:**
1. **source**: `M` (molecule) or `T` (table)
2. **identifier**: molecule_key or table_name
3. **field**: Which field to return (label/code/value for molecules, column_name for tables)
4. **length**: Optional max length (truncate, don't pad)
5. **case**: Optional `U` (upper), `L` (lower), `P` (proper)

### Common Usage Patterns

**Error Messages:**
```javascript
const msg = "Member does not have enough {{M,point_type,label,,L}}";
await resolveAtoms(msg, context);
// → "Member does not have enough miles"
```

**Display Templates:**
```javascript
const template = "{{T,members,first_name,,P}}, you have {{M,point_type,label,,L}}!";
await resolveAtoms(template, context);
// → "Bill, you have miles!"
```

**Dynamic Headers:**
```javascript
const header = "Total {{M,point_type,label,,P}} Earned";
await resolveAtoms(header, context);
// → "Total Miles Earned"
```

### The Two Functions

**resolveAtom(atomString, context)**
- Resolves a single atom
- Returns: string value
- Use when: Processing one atom at a time

**resolveAtoms(template, context)**
- Finds and resolves all atoms in a string
- Returns: Complete string with atoms replaced
- Use when: Processing error messages, templates, any multi-atom text
...
const context = {
  ...
  memberId: '2153442807',   // Required for member table lookups
  activityId: 15,           // Required for activity table lookups
  dbClient: dbClient,       // Required for table lookups
  molecules: {...}          // Optional: pre-decoded molecules for efficiency
};
```
...
- Error messages that include labels
- Display text that references molecules
- Templates that show member/activity data
- Any text that needs to adapt per tenant

❌ **NEVER do these:**
- Hardcode "miles", "points", "Flight", etc. in strings
- Query `molecule_def` directly for labels
- Query tables directly for display values
- Use string concatenation with hardcoded labels

### Implementation Location

File: `/home/claude/loyalty-demo/atom_resolve.js`
Functions: `resolveAtom()`, `resolveAtoms()`
