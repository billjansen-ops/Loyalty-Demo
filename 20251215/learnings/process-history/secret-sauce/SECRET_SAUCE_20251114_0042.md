# SECRET SAUCE
**Updated:** November 14, 2025 00:42
**Version:** 20251114_0042
...
### 8. Molecule Type Taxonomy (November 13, 2025) 🆕
...
├── Single Value: "Miles", 365, true
└── Embedded List: Categories with values (activity_display)

DYNAMIC - User-entered or frequently changing data
├── Lookup (Internal): Pre-defined list (fare classes)
├── Lookup (External): References table (carriers, airports)
└── Freeform: User types value (confirmation numbers)

REFERENCE - Computed/derived values
├── Direct Field: Query table.column (member.fname)
└── Function: Call stored function (get_tier_on_date)
```
...
[Apply Bonus Now]  ← CSR can manually apply
```
...
## 🔧 LONGORIA Command (November 12-13, 2025) 🆕
...
## 💾 Session Handoff System (November 12-13, 2025) 🆕
...
```
END OF CHAT:
├── START_CHAT_INSTRUCTIONS_YYYYMMDD_HHMM.md (How to boot)
├── SESSION_SUMMARY_YYYYMMDD_HHMM.md (What happened today)
├── WORKFLOW_STANDARDS_YYYYMMDD_HHMM.md (How we work) ← ACCUMULATES
└── SECRET_SAUCE_YYYYMMDD_HHMM.md (Why this is brilliant) ← ACCUMULATES

Bill runs: create_handoff_package.sh
├── Captures schema
├── Copies all files (including above 4)
└── Creates tarball

START NEW CHAT:
├── Upload START_CHAT_BOOTSTRAP.md
├── Upload handoff tarball
├── New Claude extracts
├── Uses `ls -t` to find latest versions
└── Reads all 4 files
```
...
Post old activities → they find correct buckets based on activity_date
No batch processing needed

### Discovery 3: Display Templates Are Just Molecule Maps (November 10, 2025)
```javascript
{
  "origin": "Origin",      // Molecule key → Display label
  "dest": "Destination",
  "carrier": "Carrier"
}
```
...
### Discovery 7: Progressive Disclosure for Molecule Edit (November 13, 2025) 🆕
...
**The pattern:**
```javascript
// Type changes → show Type config section
...
// Subtype changes → show Subtype fields
...
### Discovery 8: Endpoint Design Philosophy (November 13, 2025) 🆕
...
### Discovery 9: Reference Type Implementation (November 13-14, 2025) 🆕
...
- External Lookup: Points to TABLE with MANY rows (carriers, airports) → Returns list of choices
- Reference/Direct Field: Points to SINGLE FIELD in specific row (member.fname) → Returns one value
- Reference/Function: Calls stored function → Returns computed value

### Discovery 10: Consistent Value Loading Pattern (November 13-14, 2025) 🆕
**Problem:** When editing molecules, some types loaded their values, others didn't.

**The systematic fix:**
```javascript
// Static/Single Value
loadScalarValue(moleculeId) → GET /v1/molecules/:id/value

// Static/Embedded List  
loadEmbeddedCategories(moleculeId) → GET /v1/molecules/:id/embedded-categories
loadCategoryValues(category) → GET /v1/molecules/:id/embedded-values?category=X

// Dynamic/Lookup Internal
loadListValues(moleculeId) → GET /v1/molecules/:id/values

// Dynamic/Lookup External
loadLookupConfig(moleculeId) → GET /v1/molecules/:id/lookup-config
...
### Discovery 11: Handoff File Timing (November 13-14, 2025) 🆕
