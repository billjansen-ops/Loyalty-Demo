...
-- Storage: 50,000 × 4 bytes = 200 KB (vs. 150 KB raw, but with index overhead)
-- Plus one 'MSP' + 'Minneapolis' (~20 bytes)
```

**Benefit:** Update "Minneapolis" to "Minneapolis-St Paul" → ALL activities reflect it
...
## 📜 Historical Context - "A Million Years Ago"
...
- Update molecule value → all activities reflect it

**It's the same philosophy Bill used "a million years ago"**

---

## 🔍 Key Discoveries From Sessions
...
**No batch windows:**
- Post activity → immediate balance update
- Earn bonus → immediate credit
- Expire points → derive on-demand
- Change tier → instant reflection
...
1. **It's temporal-first** → Retro and future just work
2. **It's pointer-based** → Fast and space-efficient
3. **It's data-driven** → Code never changes
4. **It's industry-agnostic** → Same tables for all verticals
5. **It's self-documenting** → Metadata explains itself
6. **It derives on-demand** → No batch jobs
7. **It right-sizes types** → Cache-friendly
8. **It's immutable** → Audit trail built-in

**The real magic:**
Not any one technique, but how they combine:
- Temporal + Pointers → Efficient history
- Metadata + Universal Functions → Industry agnostic
- Derive + Temporal → No batch jobs
- Right-sizing + Pointers → Cache friendly
...
## 🔍 This Session (Nov 7, 2025)

### Display Template System Architecture

**New Capability:** Dynamic activity display composition

**Core Insight:** Instead of hardcoding how activity data displays, store template definitions that compose molecule references into display strings.

**Why It's Brilliant:**
- Same template system works for flights, hotels, retail (molecule abstraction)
- Tenants customize display without code changes (data-driven)
- Preview uses sample data from molecule definitions (self-contained)
- Templates reference molecules, so they're automatically validated
...
display_template (
  ...
  tenant_id,
  template_name,
  template_type,  -- V (Verbose) or E (Efficient) from molecule
  is_active       -- Only one active per type per tenant
)

display_template_line (
  line_id,
  template_id,
  line_number,    -- For sorting
  template_string -- The magic: [M,carrier,"Code"],[T," * "]
)
```
...
- Molecules already exist (carrier, origin, destination)
- Format options mirror decode needs (Code, Description, Both)
- Text components allow static separators
- Max length prevents display overflow
- Parser can validate molecule existence

### Function Extensibility Pattern

**Discovery:** Optional parameters with string enums are better than booleans

**Before (hypothetical):**
```javascript
getMolecule(key, tenantId, includeSamples)  // Boolean - limited
```

**After (implemented):**
```javascript
getMolecule(key, tenantId, returnType = 'standard')  // String enum - extensible
```

**Future possibilities:**
- `'with_samples'` - Current
- `'with_metadata'` - All def fields
- `'with_values'` - Plus value lists
- `'full'` - Everything
- `'minimal'` - Just key and label

**Why It's Better:**
- Self-documenting (string describes what you get)
- Easily extensible (add new modes)
- Backward compatible (default = 'standard')
- No boolean blindness (true/false unclear)

### Sample Data as Documentation

**Pattern:** molecule_def includes sample_code and sample_description

**Why This Matters:**
1. **Self-Documenting:** Each molecule shows example values
2. **Preview-Friendly:** UI can show realistic preview without live data
3. **Test-Friendly:** Automated tests can use sample data
4. **Tenant-Specific:** Each tenant's sample data reflects their domain

**Example:**
```sql
-- Airlines
carrier: sample_code = 'DL', sample_description = 'Delta Air Lines'

-- Hotels (future)
hotel_brand: sample_code = 'HLTN', sample_description = 'Hilton'

-- Retail (future)
merchant: sample_code = 'AMZN', sample_description = 'Amazon.com'
```

**The Brilliance:** System documents itself through actual data, not comments.

### Line Number Ordering Pattern

**Rediscovered:** "Million years ago" technique for reordering without drag-and-drop

**Pattern:**
- Items numbered by 10s (10, 20, 30, 40...)
- User can insert between by typing intermediate value (15)
- User can move to top by typing smaller number (5)
- User can move to end by typing larger number (100)
- System re-sorts after edit
...
- Keyboard-friendly
- Clear mental model
- Same as BASIC line numbers (Bill's era)

**Implementation:**
```javascript
// Default increment
const newLineNumber = lines.length > 0 
  ? Math.max(...lines.map(l => l.line_number)) + 10 
  : 10;
...
**Discovered:** Line Builder as separate helper for complex input

**Problem:** Template string syntax is powerful but not user-friendly to type manually

**Solution:** Visual builder constructs string programmatically

**Pattern:**
1. Main editor shows template strings (readonly display)
2. Click Add/Edit opens Line Builder helper
3. Helper shows components as list with line numbers
4. Add Molecule button → form with dropdowns
5. Add Text button → simple text input
6. Helper builds template string from components
7. Save returns string to main editor

**Why This Works:**
- Power users can still type strings directly
- Regular users get visual builder
- Helper focuses on single line at a time
- Component list with line numbers for reordering
- Preview shows what line will look like

**The Brilliance:** Two interfaces for same data - expert mode and guided mode.

---

## 💡 Key Discoveries

### Dynamic Display Composition

**Realization:** Activity display is just another form of molecule decoding

**Activity Data Flow:**
1. Store: activity_detail → (molecule_id, v_ref_id)
2. Decode: v_ref_id → (code, description)
3. Display: template → composed string

**Display Template Flow:**
1. Define: template_line → "[M,carrier,"Code"]"
2. Parse: Extract molecule_key and format
3. Render: Fetch sample/actual data → compose display

**They're Parallel:** Both use molecules to avoid hardcoding domain specifics.

### Sample Data as First-Class Citizen

**Traditional Approach:**
- Test data is separate from schema
- Mock services for preview
- Fixtures in test suites

**This Approach:**
- Sample data lives WITH definition
- No mocking needed for preview
- Same data for tests and UI
- Tenant-specific examples

**Impact:** Every molecule is self-contained and self-documenting.

### Function Parameters As API Design

**Learning:** How you design function parameters affects long-term maintainability

**Good Pattern:**
```javascript
function(required, required, optional = default)
```

Where `optional` is:
- String enum (not boolean)
- Descriptive values
- Default maintains current behavior

**Bad Pattern:**
```javascript
function(required, required, flag1, flag2, flag3)
```

Where flags are:
- Booleans (hard to extend)
- Positional (hard to remember)
- Combinatorial (too many states)

**Rule:** If you might want "one more option" later, use string enum now.
...
1. **Misunderstood Line Builder scope** - Put line numbers on wrong screen
2. **Assumed backend worked** - Didn't verify sample data was saving
3. **Didn't read chat carefully** - When asked to review, missed key context
4. **Overconfident without testing** - Claimed things worked that didn't

### Corrections Applied

1. **Read schema before coding** - Checked display_template structure first
2. **Used helper function pattern** - Extended getMolecule properly
3. **Tested incrementally** - Verified sample data at each layer
4. **Listened to frustration** - Recognized "you are so frustrating" as stop signal

### What Worked Well

1. **getMolecule() enhancement** - Clean, backward compatible, extensible
2. **Sample data integration** - Smooth from schema to preview
3. **Template preview** - Auto-updating works well
4. **CSS compression** - Vertical space optimization successful

### Carrying Forward

**Remember:**
- Read thoroughly when asked
- Verify at all layers
- Don't assume, test
- Frustration signals = stop and reconsider
- Helper functions > direct SQL
- String enums > booleans
...
**What This Enables:**
- Activity display templates (current)
- Member profile templates (future)
- Email notification templates (future)
- Report format templates (future)

**Same Pattern:**
- Template references molecules
- Molecules provide data
- Sample data enables preview
- Syntax is parseable and validatable

### Industry Agnostic (Again)

**Display Templates + Molecules = Universal Display System**

Airlines:
```
[M,carrier,"Code"],[T," * "],[M,origin,"Code"],[T,"-"],[M,destination,"Code"]
```

Hotels:
```
[M,hotel_brand,"Code"],[T," • "],[M,city,"Description"],[T," ("],[M,check_in,"Code"],[T,")"]
```

Retail:
```
[M,merchant,"Description"],[T," - "],[M,category,"Description"],[T," $"],[M,amount,"Code"]
```

**Same code. Different molecules. Different displays.**

---

**The pattern emerges clearer:** Everything is molecules. Activities, members, bonuses, templates - all reference molecules. The system becomes more uniform, not more complex, as it grows.
