 ...
 * @param {any} value - Human value (e.g., 'MSP', 'C', 1247, 'Minnesota')
 * @param {boolean} returnTextValue - For list type: if true, match display_label and return text_value (e.g., 'Minnesota' -> 'MN')
 * @returns {Promise<number|string>} - Integer ID to store, or text_value if returnTextValue=true
 */
async function encodeMolecule(tenantId, moleculeKey, value, returnTextValue = false) {
  ...
  if (mol.value_kind === 'list') {
    if (returnTextValue) {
      // Match on display_label, return text_value (e.g., 'Minnesota' -> 'MN')
      const listQuery = `
        SELECT text_value
        FROM molecule_value_text
        WHERE molecule_id = $1 AND display_label = $2
      `;
      
      const listResult = await dbClient.query(listQuery, [mol.molecule_id, value]);
      
      if (listResult.rows.length === 0) {
        throw new Error(`Display label '${value}' not found in list for molecule '${moleculeKey}'`);
      }
      
      return listResult.rows[0].text_value;
    } else {
      // Match on text_value, return value_id (e.g., 'MN' -> 27)
      const listQuery = `
        SELECT value_id
        FROM molecule_value_text
 ...
 * @param {number} tenantId - Tenant ID
 * @param {string} moleculeKey - Molecule key (e.g., 'origin', 'fare_class')
 * @param {number|string} inputValue - Integer ID from child record, or text_value if returnDisplayLabel=true
 * @param {boolean} returnDisplayLabel - For list type: if true, match text_value and return display_label (e.g., 'MN' -> 'Minnesota')
...
async function decodeMolecule(tenantId, moleculeKey, inputValue, returnDisplayLabel = false) {
  ...
  if (mol.value_kind === 'list') {
    if (returnDisplayLabel) {
      // Match on text_value, return display_label (e.g., 'MN' -> 'Minnesota')
      const listQuery = `
      ...
      const listResult = await dbClient.query(listQuery, [mol.molecule_id, inputValue]);
      
      if (listResult.rows.length === 0) {
        throw new Error(`Text value '${inputValue}' not found in list for molecule '${moleculeKey}'`);
      }
      
      return listResult.rows[0].display_label;
    } else {
      // Match on value_id, return text_value (e.g., 27 -> 'MN')
      const listQuery = `
        SELECT text_value
        FROM molecule_value_text
      ...
      if (listResult.rows.length === 0) {
        throw new Error(`Value ID ${inputValue} not found in list for molecule '${moleculeKey}'`);
      }
  ...
  if (mol.value_kind === 'scalar') {
    ...
    if (mol.scalar_type === 'text') {
      ...
      const textResult = await dbClient.query(textQuery, [inputValue]);
      
      if (textResult.rows.length === 0) {
        throw new Error(`Text ID ${inputValue} not found in text pool`);
      }
    ...
    if (mol.scalar_type === 'numeric') {
      return inputValue; // The inputValue IS the value for numeric scalars
    }
