...
const ActivityInputFields = {
  ...
  renderHTML(options = {}) {
    ...
    html += `
      ...
      <div class="${cssClass}">
        <div class="form-group">
          <label class="form-label" for="basePoints" id="basePointsLabel">Base Miles</label>
          <input class="form-input" type="number" id="basePoints" value="1000" min="0" required style="max-width: 180px;">
        </div>
      </div>
    `;
  ...
  async loadMoleculeDropdown(moleculeKey, selectId, labelId) {
    try {
      ...
      let values = [];
      const vk = moleculeDef.value_kind;
      
      // Load values based on molecule type (support both old and new names)
  ...
  async updateBaseMilesLabel() {
    try {
      ...
      const labelElement = document.getElementById('basePointsLabel');
  ...
  validate() {
    ...
    if (!data.base_points || data.base_points <= 0) errors.push('Base miles must be greater than 0');
