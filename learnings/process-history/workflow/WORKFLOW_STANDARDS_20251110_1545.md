...
├── server_db_api.js          # Main API server (5032 lines)
...
- Naming: Descriptive names (create_redemption_molecule.sql, add_column_name.sql)
- Always include tenant isolation in WHERE clauses

### Learning Files
- Location: `~/Projects/Loyalty-Demo/learnings/`
- Naming: `[TYPE]_[YYYYMMDD_HHMM].md`
- Find latest: `ls -t ~/Projects/Loyalty-Demo/learnings/[TYPE]_*.md | head -1`
...
**CRITICAL - Always use these:**
- **Database name**: `loyalty` (NOT "loyalty_platform")
- **Username**: `billjansen` (NOT "admin")
- **Host**: `localhost`
- **Connection**: `psql -h localhost -U billjansen -d loyalty`
...
activity_type       CHAR(1)         -- 'A', 'R', 'X', etc. (single character)
```
...
**Never add columns to activity table for activity-specific data**

Use activity_detail with molecules instead:
```sql
-- Wrong approach (DO NOT DO THIS)
ALTER TABLE activity ADD COLUMN redemption_rule_id INTEGER;

-- Right approach (THE MOLECULE WAY)
-- 1. Create molecule (if needed)
INSERT INTO molecule_def VALUES (tenant_id, 'redemption', 'Redemption Type', 'lookup', 'activity');

-- 2. Store in activity_detail
INSERT INTO activity_detail (activity_id, molecule_id, v_ref_id)
VALUES (activity_id, redemption_molecule_id, redemption_rule_id);

-- 3. Decode when loading
SELECT ... FROM activity_detail WHERE molecule_key = 'redemption';
```

This pattern applies to: carrier, origin, destination, redemption, hotel_brand, etc.
...
- **Gets frustrated with**:
  ...
  - Providing partial files or asking Bill to edit
  - Guessing at database names, column names, etc.
  - Wasting time on troubleshooting preventable issues
...
- **Appreciates**:
  ...
  - Checking schema before assuming
...
- **Clean data**: Will wipe tables and start fresh if data is messy
...
### Schema & Database
- **ALWAYS read schema** before writing SQL or code
- **NEVER guess** column names, table names, or data types
- **ALWAYS verify** with grep or schema file
- **ALWAYS provide complete SQL commands** including database and user
...
ls -t ~/Projects/Loyalty-Demo/learnings/SESSION_SUMMARY_*.md | head -1
...
# Search for pattern in schema
grep -n "pattern" ~/Projects/Loyalty-Demo/learnings/schema_snapshot.sql
```

### Data Cleanup
```bash
# Clean all activity data
psql -h localhost -U billjansen -d loyalty -c "DELETE FROM redemption_detail; DELETE FROM activity_bonus; DELETE FROM activity_detail; DELETE FROM activity; DELETE FROM point_lot;"
```
...
### Color Coding
- **Green** (#059669): Positive actions, additions, success, flights
...
### Layout Lessons
- **Avoid CSS Grid for simple spacing** - Use margin-left or flexbox instead
- **Test whitespace early** - CSS can behave unexpectedly across browsers
- **Simple beats complex** - margin-left: 20px > grid-template-columns: 1fr auto
...
- camelCase for variables (but match API: tenant_id vs tenantId)
...
- **Display showing raw values** → Decoding not working, check molecule
- **"tenantId is not defined"** → Variable name mismatch (tenantId vs tenant_id)
...
```javascript
// Centralized molecule fetching with category support
async function getMolecule(moleculeKey, tenantId, category = null) {
  // Supports embedded_list with category parameter
  const query = `SELECT md.*, ... WHERE md.tenant_id = $1 AND md.molecule_key = $2`;
  // Returns parsed molecule with values array
}
```

### Activity Detail Pattern (CRITICAL)
```javascript
// When processing activity, store molecule references
const moleculeId = await getMoleculeId(tenantId, 'redemption');
await dbClient.query(
  'INSERT INTO activity_detail (activity_id, molecule_id, v_ref_id) VALUES ($1, $2, $3)',
  [activityId, moleculeId, redemptionRuleId]
);

// When loading activity, decode molecules
const details = await getActivityDetails(activityId);
for (const detail of details) {
  decodedValues[detail.molecule_key] = await decodeMolecule(
    tenantId, 
    detail.molecule_key, 
    detail.v_ref_id
  );
...
### SQL File Execution
**Problem:** Spent 30+ minutes troubleshooting SQL because of wrong database/user names
**Solution:** ALWAYS provide complete command:
```bash
cd ~/Projects/Loyalty-Demo && psql -h localhost -U billjansen -d loyalty -f sql/filename.sql
```

### Column vs Molecule Decision
**Problem:** Almost added redemption_rule_id column to activity table
**Solution:** Use activity_detail with molecules - that's the pattern!
**Rule:** Never add columns for activity-type-specific data

### CSS Grid Whitespace
**Problem:** CSS Grid creating excessive whitespace despite various column settings
**Solution:** Use simple margin-left instead of grid for small spacing needs
**Rule:** Simple beats complex for basic layouts

### Variable Name Consistency
**Problem:** tenantId vs tenant_id causing "not defined" errors
**Solution:** Match the variable names from req.body or function parameters
**Rule:** Be consistent with naming, especially camelCase vs snake_case
