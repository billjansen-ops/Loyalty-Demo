...
const SERVER_VERSION = "2026.01.05.1830";
const BUILD_NOTES = "Added getMemberBadgeOnDate function and wiring for member_badge_on_date molecule";
...
async function evaluateCriteria(ruleId, activityData, memberLink, tenantId, activityDate, failFast = true) {
  ...
  for (const criterion of criteriaResult.rows) {
    ...
    // Get molecule definition (case-insensitive)
    const molDefQuery = `
      SELECT value_kind, scalar_type, lookup_table_key
      FROM molecule_def
      WHERE LOWER(molecule_key) = LOWER($1) AND tenant_id = $2
    `;
    const molDefResult = await dbClient.query(molDefQuery, [criterion.molecule_key, tenantId]);
    ...
    if (isLookupMolecule(moleculeDef)) {
      ...
      `;
      const lookupConfigResult = await dbClient.query(lookupConfigQuery, [criterion.molecule_key, tenantId]);
...
async function getMoleculeStorageInfo(tenantId, moleculeKey, columnOrder = 1) {
  // Get molecule_id from header (still needed for FK relationship)
  const headerResult = await dbClient.query(`
    SELECT molecule_id
    FROM molecule_def
    WHERE LOWER(molecule_key) = LOWER($1) AND (tenant_id = $2 OR tenant_id IS NULL)
  ...
  const moleculeId = headerResult.rows[0].molecule_id;
  
  // Get all data from child table (molecule_value_lookup)
  const childResult = await dbClient.query(`
    SELECT value_type, context, storage_size, attaches_to, lookup_table_key,
           ref_table_name, ref_field_name, ref_function_name
    FROM molecule_value_lookup
    WHERE molecule_id = $1 AND column_order = $2
  `, [moleculeId, columnOrder]);
  
  if (childResult.rows.length === 0) {
    throw new Error(`Molecule ${moleculeKey} column ${columnOrder} not found in molecule_value_lookup`);
  }
  
  const { value_type, context, storage_size, attaches_to } = childResult.rows[0];
  const tableName = getDetailTableName(context, storage_size);
  const columns = parseStoragePattern(String(storage_size));
  
  // For multi-column molecules, get value_type for each column from its row
  if (columns.length > 1) {
    const allColsResult = await dbClient.query(`
      SELECT column_order, value_type
      FROM molecule_value_lookup
      WHERE molecule_id = $1
    ...
    for (let i = 0; i < columns.length; i++) {
      const colRow = allColsResult.rows[i];
      if (colRow) {
        columns[i].valueType = colRow.value_type || null;
      } else {
        columns[i].valueType = value_type || null;
      }
    }
  } else {
    // Single column molecule - use the value_type from row 1
    columns[0].valueType = value_type || null;
  }
  
  return { moleculeId, context, storageSize: storage_size, valueType: value_type, tableName, columns, attachesTo: attaches_to };
}
...
async function loadCaches() {
  ...
  try {
    ...
    // is_deleted molecule cache - REQUIRED for soft delete functionality
    ...
    debugLog(`   ✓ is_deleted molecule: ${isDeletedResult.rows.length} tenants`);
    
    // Verify all tenants have is_deleted molecule
    const tenantResult = await dbClient.query('SELECT tenant_id FROM tenant');
    const missingTenants = tenantResult.rows
      .filter(t => !caches.isDeletedMoleculeId.has(t.tenant_id))
      .map(t => t.tenant_id);
    
    if (missingTenants.length > 0) {
      console.error(`❌ FATAL: is_deleted molecule missing for tenant(s): ${missingTenants.join(', ')}`);
      console.error('   Run: psql -d loyalty -f sql/flag_molecules.sql');
      process.exit(1);
    }
...
app.delete('/v1/members/:memberId/badges/:badgeId', async (req, res) => {
  ...
  try {
    ...
    // Delete by p_link, molecule_id, and N1 (badge_id)
    // Need to encode badge_id for comparison
...
app.get("/v1/member/:id/info", async (req, res) => {
  ...
  try {
    ...
    // Get current tier
    const tierResult = await getMemberTierOnDate(memberLink, new Date().toISOString().slice(0, 10), tenantId);
...
app.post('/v1/test-rule/:bonusCode', async (req, res) => {
  ...
  try {
    ...
    const tenantId = parseInt(req.query.tenant_id) || req.body.tenant_id || 1;
          ...
          for (const result of results) {
            ...
            if (result.result_type === 'points' && result.result_amount > 0) {
              ...
              if (existingEnroll.rows.length === 0) {
                // Get target promotion's goal_amount
                const targetPromo = await dbClient.query(
                  `SELECT goal_amount FROM promotion WHERE promotion_id = $1`,
                  [result.result_reference_id]
                );
                const goalAmount = targetPromo.rows[0]?.goal_amount || 1;
...
async function evaluateEnrollmentPromotions(memberLink, tenantId, enrollDate, memberData = {}) {
  ...
  try {
    ...
    // Get active promotions with count_type = 'enrollments'
    const promotionQuery = `
      SELECT promotion_id, promotion_code, promotion_name, enrollment_type,
             count_type, goal_amount, reward_type, reward_amount, 
             reward_tier_id, reward_promotion_id, rule_id,
             start_date, end_date,
             duration_type, duration_days, duration_end_date
      FROM promotion 
      WHERE tenant_id = $1
        AND is_active = true 
        AND count_type = 'enrollments'
      ORDER BY promotion_code
    `;
    const promotionResult = await dbClient.query(promotionQuery, [tenantId]);
    const enrollmentPromotions = promotionResult.rows;
    ...
    if (existing.rows.length > 0) {
      ...
      const updateQuery = `
        ...
        SET bonus_description = $1,
            ...
            apply_friday = $13,
            apply_saturday = $14
        WHERE bonus_code = $15 AND tenant_id = $7
        RETURNING *
      `;
      const updateParams = [
        bonus_description,
...
async function evaluateTokenActivity(tokenActivityLink, adjustmentId, memberLink, tenantId, activityDate, depth = 0) {
  ...
  try {
    // Find active promotions that count this token type
    const tokenPromosQuery = `
      SELECT p.*, mp.member_promotion_id, mp.progress_counter, mp.goal_amount as enrolled_goal
      FROM promotion p
      LEFT JOIN member_promotion mp ON p.promotion_id = mp.promotion_id AND mp.p_link = $1 AND mp.qualify_date IS NULL
      WHERE p.tenant_id = $2
        AND p.is_active = true
        AND p.count_type = 'tokens'
        AND p.counter_token_adjustment_id = $3
        AND $4::date BETWEEN p.start_date AND p.end_date
    `;
    
    const tokenPromosResult = await dbClient.query(tokenPromosQuery, [memberLink, tenantId, adjustmentId, activityDate]);
    
    if (tokenPromosResult.rows.length === 0) {
      debugLog(() => `      → No token-counting promotions found for this token type`);
      return;
    }
    
    debugLog(() => `      → Found ${tokenPromosResult.rows.length} token-counting promotion(s)`);
    
    for (const promo of tokenPromosResult.rows) {
      ...
      if (newProgress >= goalAmount) {
        ...
        // Update qualify_date
        await dbClient.query(
          `UPDATE member_promotion SET qualify_date = $1 WHERE member_promotion_id = $2`,
        ...
        if (hasProcessableResults) {
          await dbClient.query(
            `UPDATE member_promotion SET process_date = $1 WHERE member_promotion_id = $2`,
            [activityDate, memberPromotionId]
          );
        }
      }
  ...
  if (result.result_type === 'points' && result.result_amount > 0) {
    ...
    const cascadeQuery = `
      ...
      WHERE mp.promotion_id = p.promotion_id
        ...
        AND pr.result_type = 'tier'
        AND pr.result_reference_id = $3
        AND p.promotion_id != $4
        AND (
          -- Calendar type: end date must be <= this tier's end date
          (pr.duration_type = 'calendar' AND pr.duration_end_date <= $5)
          OR
          -- Virtual type: duration must be <= this tier's duration
          (pr.duration_type = 'virtual' AND pr.duration_days <= $6)
        )
    `;
    ...
    const cascadeResult = await client.query(cascadeQuery, [
      ...
      memberLink,                        // $2 - p_link
      result.result_reference_id,        // $3 - same tier only
      promotionId,                       // $4 - qualified_by_promotion_id
      endDate,                           // $5 - for calendar comparison
      result.duration_days || 0          // $6 - for virtual comparison
    ]);
    ...
    if (existingEnroll.rows.length === 0) {
      // Get target promotion's goal_amount
      const targetPromo = await client.query(
        `SELECT goal_amount FROM promotion WHERE promotion_id = $1`,
        [result.result_reference_id]
      );
      const goalAmount = targetPromo.rows[0]?.goal_amount || 1;
...
async function getMemberTierOnDate(memberLink, date, tenantId) {
  const query = `
    SELECT 
      ...
      td.tier_ranking
    FROM member_tier mt
    JOIN tier_definition td ON mt.tier_id = td.tier_id AND td.tenant_id = $3
    WHERE mt.p_link = $1
  ...
  const result = await dbClient.query(query, [memberLink, date, tenantId]);
 ...
 */
async function getMemberBadgeOnDate(memberLink, date, tenantId) {
  ...
  // Return comma-separated list of badge codes
      ...
      if (cached.ref_function_name) {
        ...
        if (cached.ref_function_name === 'get_member_badge_on_date' && context.member_link && date) {
          try {
            const memberLink = context.member_link;
            if (memberLink) {
              const badgeCodes = await getMemberBadgeOnDate(memberLink, date, tenantId);
              return badgeCodes; // Returns comma-separated list or null
            }
          } catch (error) {
            return null;
          }
...
app.post('/v1/redemptions/process', async (req, res) => {
  ...
  try {
    ...
    debugLog(`✓ Processed redemption for member ${member_id}: ${point_amount} points from ${breakdown.length} bucket(s)`);
    
    // Record redemption statistics
    await recordRedemptionRedeemed(redemption_rule_id, point_amount, activityDate);
  ...
  try {
    ...
    const query = `
      UPDATE promotion_result SET
        ...
        duration_days = $9,
        sort_order = $10
      WHERE promotion_result_id = $1 AND promotion_id = $2 AND tenant_id = $11
      RETURNING *
    `;
    ...
    const values = [
      ...
      result_reference_id, result_description,
      duration_type, duration_end_date, duration_days, sort_order, tenant_id
    ];
...
async function runStressTestJob(jobId) {
  ...
  try {
    ...
    const fareClassResult = await dbClient.query(`
      ...
      WHERE UPPER(md.molecule_key) = 'FARE_CLASS' AND md.tenant_id = $1
    `, [tenantId]);
